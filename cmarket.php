...
function cmarket_index(): void
{
    ...
    $q = $db->run(
        'SELECT cm.cmADDER, cm.cmPRICE, cmID, cmQTY, u.userid, username, level, money, crystals, gender, donatordays
        FROM crystalmarket AS cm
        LEFT JOIN users AS u ON u.userid = cm.cmADDER
        ORDER BY (cmPRICE/cmQTY)',
    );
    foreach ($q as $r) {
...
function crystal_remove(): void
{
    ...
    $_GET['ID'] =
        (isset($_GET['ID']) && is_numeric($_GET['ID']))
            ? abs(intval($_GET['ID'])) : '';
    if (empty($_GET['ID'])) {
        echo 'Something went wrong.<br />&gt; <a href="cmarket.php" title="Go Back">Go Back</a>';
        $h->endpage();
        exit;
    }
    $r = $db->row(
        'SELECT cmQTY FROM crystalmarket WHERE cmID = ? AND cmADDER = ?',
        $_GET['ID'],
        $userid,
    );
    if (empty($r)) {
...
function crystal_add(): void
{
	...
	<table width="35%" align="center" style="border: none; padding: 1px;" cellspacing="1">
	<tr>
