...
$q = $db->run(
    'SELECT fed_userid, fed_days, fed_reason, fed_jailedby, u.username, u2.username AS jailer
    FROM fedjail AS f
    LEFT JOIN users AS u ON f.fed_userid = u.userid
    LEFT JOIN users AS u2 ON f.fed_jailedby = u2.userid
    ORDER BY f.fed_days',
);
...
echo '</table>';
$q = $db->run(
    'SELECT userid, username, mailban, mb_reason FROM users WHERE mailban > 0 ORDER BY mailban'
);
