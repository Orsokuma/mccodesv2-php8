...
Most hated: [";
    $q2r = $db->run(
        'SELECT username, userid FROM users ORDER BY friend_count DESC LIMIT 5'
    );
    $r   = 0;
    foreach ($q2r as $r2r) {
        $r++;
...
<table width="90%" class="table" cellspacing="1">
   ...
   ';
    $q = $db->run(
        'SELECT fl_COMMENT, fl_ID, laston, donatordays, username, userid
    ...
    foreach ($q as $r) {
        ...
        if ($r['donatordays'] > 0) {
            ...
            $d             =
                "<img src='app/view/assets/images/donator.gif' alt='Donator: {$r['donatordays']} Days Left'
...
function add_friend(): void
{
    ...
    if ($_POST['ID']) {
        $dupe_count = $db->cell(
            'SELECT COUNT(fl_ADDER) FROM friendslist WHERE fl_ADDER = ? AND fl_ADDED = ?',
            $userid,
            $_POST['ID'],
        );
        $r          = $db->row(
...
&gt; <a href="friendslist.php">Back</a>
   ';
    ...
    $r = $db->row(
        'SELECT fl_ID, fl_ADDED FROM friendslist WHERE fl_ID = ? AND fl_ADDER = ?',
        $_GET['f'],
        $userid,
    );
    if (empty($r)) {
...
function change_comment(): void
{
    ...
    if ($_POST['comment'] && $_POST['f']) {
        $exists = $db->exists(
            'SELECT COUNT(fl_ID) FROM friendslist WHERE fl_ID = ? AND fl_ADDER = ?',
            $_GET['f'],
            $userid,
        );
        if (!$exists) {
...
<a href='friendslist.php'>&gt; Back</a>
   ";
        ...
        $r = $db->row(
            'SELECT fl_COMMENT FROM friendslist WHERE fl_ID = ? AND fl_ADDER = ?',
            $_GET['f'],
            $userid,
        );
        if (!empty($r)) {
