...
check_access('manage_challenge_bots');
...
function addbot(): void
{
    ...
    if ($_POST['userid'] && $_POST['money']) {
        ...
        $r = $db->row(
            'SELECT user_level, userid, username FROM users WHERE userid = ?',
            $_POST['userid'],
        );
        if (empty($r)) {
        ...
        $exists = $db->exists(
            'SELECT COUNT(cb_npcid) FROM challengebots WHERE cb_npcid = ?',
            $r['userid'],
        );
        if ($exists) {
